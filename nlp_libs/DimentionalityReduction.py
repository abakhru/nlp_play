...
"""
Here we are implementing some dimensionality reduction for the auto-insurance prediction problem
"""
...
# define function for getting correlation variables
...
print(f"Binary colums={len(bin_cols)} , "
      f"Categorical columns={len(cat_cols)}, Numeric columns={len(num_cols)}")
...
train_df, test_df, label_train, label_test = train_test_split(df, label, random_state=101,
                                                              stratify=label, test_size=0.2)
...
# First normaize the data.
# Note that Standard scaler cannot be used here since it makes all variances uniform
...
# Now we check for correlation between the nemeric
# variables and drop any variables having correlation great than 0.5 or 0.6
...
# Note - here we dont have any highly correlated features as seen in the correlation matrix
...
# Create a logistic regression model.The class_weight='balanced'
# automatically adjusts the weights for the target classes.
...
print("Training logistic reg model with selected columns ....")
